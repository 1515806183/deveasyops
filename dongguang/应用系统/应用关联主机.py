...
class HostAssociation():
    ...
    def getAppInfo(self):
        ...
        #                        "maxguage"
        #                    }},
        params = {"fields": {'name': True, "clusters.deviceList.ip": True}}
    ...
    def gevent_data(self, data):
        ...
        for clusters in clustersList:
            try:
                ...
                logging.info('应用获取集群主机信息报错:%s' % str(e))
                continue
        logging.info(str('应用:%s, 最新主机数量为:%s' % (app_name, len(LatestHostList))))

        # 会对比关系是否存在, 多余的关系移除掉，不存在的关系添加上
        url = "http://{HOST}/object/{ID}/relation/{relation_side_id}/set".format(HOST=cmdb_host,
                                                                                 ID="APP", relation_side_id="HOST")
