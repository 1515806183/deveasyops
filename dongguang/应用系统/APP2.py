...
# CMDB配置
easyops_cmdb_host = EASYOPS_LOCAL_IP
easyops_org = EASYOPS_ORG
easy_user = EASYOPS_USER
# header配置
...
class ThreadInsert(object):
    ...
    def gevent_data(self, i):
        ...
        url = '/object/{0}/relation/{1}/set'.format(ConfigSWITCHMODEL, app_and_bus_to_host)
        res = self.easyopsObj.http_post('info_set', url, app_to_host_data)
        print res
        ...
        url = '/object/{0}/relation/{1}/set'.format(BUSINESS_MODEL, app_and_bus_to_host)
        res = self.easyopsObj.http_post('info_set', url, businesses_to_host_data)
        print res
