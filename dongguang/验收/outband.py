...
MODEL_ID = 'OUT_OF_BAND_MANAGEMENT'
...
class EasyopsPubic(object):
    ...
    def search_auto_collect(self):
        params = {"query": {}, "fields": {"instanceId": True, "name": True, "diskInfo.model": True},
...
class ThreadInsert(object):
    ...
    def dealdata(self, content):
        ...
        if len(data['datas']):
            info_url = "/object/{0}/instance/_import".format("diskinfo")
            time.sleep(1)
    ...
    def gevent_data(self, data):
        ...
        model_list = data.get('diskInfo')  # 实例服务节点
        if model_list:
            for model in model_list:
                res = model.get('model')
                if res not in model_ret:
                    model_ret.append({"model": res})
