...
# 查询条件
# 搜索所有实例数据的ID
ConfigSWITCHMODEL = 'OUT_OF_BAND_MANAGEMENT'
...
class ThreadInsert(object):
    ...
    def gevent_data(self, i):
        inser_instanceId_list = []
        out_band_instanceId = i.get('instanceId')
        # 根据带外id获取下面所有的物理机和宿主机
        url = '/object/OUT_OF_BAND_MANAGEMENT/instance/' + out_band_instanceId
        res = self.easyopsObj.http_post('get', url)
        HOST_LIST = res.get('HOST', '')
        VMWARE_HOST_COMPUTER = res.get('VMWARE_HOST_COMPUTER', '')
        if HOST_LIST:
            for host in HOST_LIST:
                inser_instanceId_list.append(host.get('instanceId'))
        if VMWARE_HOST_COMPUTER:
            for VMWARE_HOST in VMWARE_HOST_COMPUTER:
                inser_instanceId_list.append(VMWARE_HOST.get('instanceId'))

        data = {
            "objectId": ConfigSWITCHMODEL,
            "instance_ids": [out_band_instanceId],
            "related_instance_ids": inser_instanceId_list
        }

        url = '/object/{0}/relation/{1}/append'.format(ConfigSWITCHMODEL, 'OUT_OF_BAND_')
        res = self.easyopsObj.http_post('info_port', url, data)
        print res
