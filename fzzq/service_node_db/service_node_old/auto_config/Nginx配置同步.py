                ...
                for key, vaule in node.items():
                    ...
                    for data in vaule:
                        ...
                        if info == 'listeners':
                            ...
                            exec_user += (data.get('value', ''),)
                        elif info == 'exe':
                            exe += (data.get('value', ''),)
                        elif info == 'version':
                            version += (data.get('value', ''),)
                        elif info == 'conf_path':
                            conf_path += (data.get('value', ''),)
                        elif info == 'worker_processes':
                            worker_processes += (data.get('value', ''),)
                        elif info == 'worker_connections':
                            worker_connections += (data.get('value', ''),)
                        elif info == 'error_log_path':
                            error_log_path += (data.get('value', ''),)
                        elif info == 'access_log':
                            access_log += (data.get('value', ''),)
                        elif info == 'vhost':
                            for vhost_data in literal_eval(data.get('value', [])):
                                vhost_data['location'] = str(vhost_data.get('location'))
                                vhost.append(vhost_data)

            res = {
                "name": name,
                "listeners": self.list_dict_duplicate_removal(listeners),
                "exec_user": tuple(set(exec_user)),
                "exe": tuple(set(exe)),
                "version": tuple(set(version)),
                "conf_path": tuple(set(conf_path)),
                "worker_processes": tuple(set(worker_processes)),
                "worker_connections": tuple(set(worker_connections)),
                "error_log_path": tuple(set(error_log_path)),
                "access_log": tuple(set(access_log)),
                "vhost": self.list_dict_duplicate_removal(vhost),
            }
