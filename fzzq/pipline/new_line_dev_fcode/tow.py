...
VID = '579454b6618bd89ae01c957f1f1c5437'  # 工具库版本ID
EXEC_USER = 'root'  # 执行账户
...
def main(target_ip):
    ...
    ret = get_tool_exec_result(exec_id)  # 获取工具的执行结果
    if ret:
        PKG_VERSION = ret['PKG_VERSION']
        UPLOAD_FILE = ret['UPLOAD_FILE']
        return {"PKG_VERSION": PKG_VERSION, "UPLOAD_FILE": UPLOAD_FILE}
...
if __name__ == '__main__':
    ...
    if PagekeVersionId == "无":
        logging.info('Start to download the package, the download process may be slow, wait patiently...')
        ret = main(SERVER_IP)
        logging.info('Download the package and return the content: %s' % ret)
