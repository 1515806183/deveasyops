...
# 需要加入白名单的组实例id, 组提前加入到对应的角色中
rold_id = ["easyops", ":5c35970e7103c"]
...
class ThreadInsert(EasyopsPubic):
    ...
    def getAppUserInfo(self):
        ...
        params = {"fields": {'name': True, 'owner.name': True,
                             'tester.name': True, "developer.name": True}}
        ...
        for data in AppInfoRetList:
            try:
                ...
                add_uesr = list(set(real_testerList + real_ownerList + real_developerList))
                # 给应用添加白名单用户
                appInfo = {
                    "instanceId": instanceId,
                    # 应用资源实例编辑
                    "updateAuthorizers": add_uesr + rold_id if add_uesr else [],
                    # 生产相关
                    "productionPipelineOperateAuthorizers": real_ownerList + rold_id if real_ownerList else rold_id,
                    # 生产流水线运维
                    "productionClusterOperateAuthorizers": real_ownerList + rold_id if real_ownerList else rold_id,
                    # 生产集群运维

                    # 测试相关
                    "testPipelineOperateAuthorizers": real_testerList + rold_id if real_testerList else rold_id,
                    # 测试流水线运维
                    "testClusterOperateAuthorizers": real_testerList + rold_id if real_testerList else rold_id,
                    # 测试集群运维

                    # 开发相关
                    "developPipelineOperateAuthorizers": real_developerList + rold_id if real_developerList else rold_id,
                    # 开发流水线运维
                    "developClusterOperateAuthorizers": real_developerList + rold_id if real_developerList else rold_id,
                    # 开发集群运维

                    # 包相关
                    "appPackageCheckPackageAuthorizers": add_uesr + rold_id if add_uesr else [],
                    "appPackageRestartAuthorizers": add_uesr + rold_id if add_uesr else [],
                    "appPackageStartAuthorizers": add_uesr + rold_id if add_uesr else [],
                    "appPackageStopAuthorizers": add_uesr + rold_id if add_uesr else [],
                    "appPackageUninstallAuthorizers": add_uesr + rold_id if add_uesr else [],
